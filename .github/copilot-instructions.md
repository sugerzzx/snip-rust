...
- Idle hover: `overlay::auto_detect` precomputes candidate rectangles. Highlight the smallest rectangle containing the cursor and treat a simple click as selecting that region without starting a drag.
...
- For manual debugging of auto-detect, run `cargo run --example auto_detect_demo` to capture the current screen, overlay detected rectangles (红色描边), and save the image to temporary directory.
